...
const SignInScreen = () => {
    ...
    const validate = (data: RegisterFormData) => {
        ...
        if (!username) {
            ...
            setError('username', { message: 'Username must not contain special characters' });
        }
        ...
        if (!username || !phone_number || !password || !confirmPassword || password.length < 6 || !usernameRegex.test(username) || !phoneRegex.test(phone_number) || password !== confirmPassword) {
            return false;
        }
        return true;
    };
                        ...
                        <View style={{ width: '100%', alignItems: 'center', marginTop: 4 }}>
                            <TouchableOpacity onPress={() => navigation.goBack()}>
                                <Text style={textStyles.body}>
                                    {t('have_account_ask')} <Text style={textStyles.pressable}>{t('sign_in')}</Text>
                                </Text>
                            </TouchableOpacity>
                        </View>
                    </View>
